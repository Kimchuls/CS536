...
void sendText(int sockfd, char *name)
{
    ...
    if (!(0 == strcmp(name, "text") || 0 == strcmp(name, "picture") || 0 == strcmp(name, "bigpicture")))
    {
        NotFound404(sockfd);
        return;
    }
...
void sendPicture(int sockfd, char *name)
{
    ...
    int tlen = sprintf(ret_buff, "HTTP/1.0 200 OK\r\n"
...
void *thread_recv(void *arg)
{
    ...
    else
    {
        ...
        *name_front = '\0';
        ...
        uri_pt++;
        if (strlen(uri) == 4 + uri_pt && uri[uri_pt] == 'h' && uri[uri_pt + 1] == 't' &&
